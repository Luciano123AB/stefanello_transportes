...
<form action="{{ route('register') }}" method="post" class="card d-grid gap-3 border border-black bg-warning shadow p-3">
    ...
    <div class="d-grid">
        ...
        <div class="input-group">
            ...
            <button type="button" class="input-group-text border-black focus-ring" wire:click="showConfirm()"><iconify-icon icon="{{ $show_confirm ? 'emojione-v1:eye' : 'simple-line-icons:eye' }}"></iconify-icon></button>
        </div>
    ...
    <div class="card-footer d-flex justify-content-between bg-warning-subtle">
        <a href="{{ route('home') }}" class="buttons_navbar_color btn btn-warning border-black align-self-baseline focus-ring focus-ring-warning">
